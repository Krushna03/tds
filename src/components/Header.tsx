...
const Header = () => {
  ...
  return (
    ...
    >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-0">
          <div className="flex justify-between items-center py-4 md:py-1">
            <div className="flex items-center">
              <img src="avenue-codeforge-removebg-preview.png" alt="tds-logo" className="h-[62px]" />
            </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 flex flex-col justify-center items-center space-y-6">
          {/* Close Button */}
          <button
            onClick={() => setIsOpen(false)}
            className="absolute top-5 right-5 p-2 rounded-md text-gray-700 dark:text-gray-300"
          >
            <X size={28} />
          </button>
          ...
          {navItems.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 text-2xl font-semibold"
              onClick={() => setIsOpen(false)}
            >
              {item.name}
            </a>
